...
## Modified Version of the original code to enable the use of the CosmosDBStorageConfig
...
from graphrag.config import create_graphrag_config as graphrag_create_graphrag_config
    ...
    with reader.envvar_prefix(Section.graphrag), reader.use(values):
        ...
        with reader.envvar_prefix(Section.storage), reader.use(values.get("storage")):
            ...
            storage_model = StorageConfig(
                ...
                database_name=reader.str("database_name", "COSMOS_DATABASE_NAME", None),
                account_host=reader.str("account_host", "COSMOS_ACCOUNT_HOST", None),
                account_key=reader.str("account_key", "COSMOS_ACCOUNT_KEY", None),
            )
